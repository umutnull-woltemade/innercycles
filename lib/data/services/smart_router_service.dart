    ...
    if (ctx.totalEntries < 1) {
      suggestions.addAll([
        const ToolSuggestion(
          ...
          reasonTr: 'İlk girişini kaydetmek için rehberli bir şablonla başla.',
          priority: 100,
          source: 'R1',
        ),
        const ToolSuggestion(
          ...
          reasonTr: 'Burada neler keşfedebileceğin için hızlı bir demo incele.',
          priority: 100,
          source: 'R1',
        ),
      ]);
    ...
    if (ctx.activeChallenge != null) {
      suggestions.add(
        ToolSuggestion(
          ...
          reasonTr: 'Aktif bir meydan okuman var. Bugünkü aksiyonu tamamla.',
          priority: 95,
          source: 'R2',
        ),
      );
    ...
    if (ctx.lastOutputType == 'entry') {
      suggestions.addAll([
        const ToolSuggestion(
          ...
          reasonTr: 'Az önce bir giriş yazdın. Kalıplarını incele.',
          priority: 90,
          source: 'R3',
        ),
        const ToolSuggestion(
          ...
          reasonTr:
              'Yeni girişin döngülerindeki eğilimler hakkında ipucu verebilir.',
          priority: 88,
          source: 'R3',
        ),
      ]);
    ...
    if (ctx.lastOutputType == 'report') {
      suggestions.addAll([
        const ToolSuggestion(
          ...
          reasonTr:
              'Raporun hazır. Kalıplarını beslemek için yeni bir giriş ekle.',
          priority: 85,
          source: 'R4',
        ),
        const ToolSuggestion(
          ...
          reasonTr: 'Son içgörünü bir kart olarak paylaş.',
          priority: 83,
          source: 'R4',
        ),
      ]);
    ...
    if (ctx.lastOutputType == 'score') {
      suggestions.addAll([
        const ToolSuggestion(
          ...
          reasonTr: 'Eğilimlerini kontrol ederek skorunun nasıl uyuyor gör.',
          priority: 80,
          source: 'R5',
        ),
        const ToolSuggestion(
          ...
          reasonTr: 'Skorunu bir meydan okumayla aksiyona dönüştür.',
          priority: 78,
          source: 'R5',
        ),
      ]);
    ...
    if (ctx.idleSeconds > 120) {
      suggestions.add(
        const ToolSuggestion(
          ...
          reasonTr:
              'Takıldım mı hissediyorsun? Rehberli bir program yardımcı olabilir.',
          priority: 75,
          source: 'R6',
        ),
      );
    ...
    if (ctx.currentStreak == 0 && ctx.totalEntries > 5) {
      suggestions.add(
        const ToolSuggestion(
          ...
          reasonTr: 'Serin durdu. Bir toparlanma görevi yardımcı olabilir.',
          priority: 65,
          source: 'R8',
        ),
      );
    ...
    if (ctx.timeBudgetMinutes <= 5) {
      suggestions.addAll([
        const ToolSuggestion(
          ...
          reasonTr:
              'Zamanın mı az? Hızlı bir şükran notu bir dakikadan kısa sürer.',
          priority: 60,
          source: 'R9',
        ),
        const ToolSuggestion(
          ...
          reasonTr: 'İki dakikalık bir nefes egzersizi her programa sığar.',
          priority: 59,
          source: 'R9',
        ),
        const ToolSuggestion(
          ...
          reasonTr: 'Bugünkü olumlamayı sadece birkaç saniyede oku.',
          priority: 58,
          source: 'R9',
        ),
      ]);
    ...
    if (ctx.userGoals.contains('discover')) {
      suggestions.add(
        const ToolSuggestion(
          ...
          reasonTr: 'Keşfetme hedefine göre yeni içgörüleri incele.',
          priority: 50,
          source: 'R10',
        ),
      );
    ...
    if (ctx.userGoals.contains('habit')) {
      suggestions.add(
        const ToolSuggestion(
          ...
          reasonTr:
              'Alışkanlık oluşturuyorsun? Ritüel yığının tutarlı kalmana yardımcı olur.',
          priority: 50,
          source: 'R11',
        ),
      );
    ...
    return manifest.relatedToolIds
        .map((relatedId) {
          ...
          return ToolSuggestion(
            ...
            reasonTr: '${manifest.labelTr} ile ilişkili.',
            priority: 40,
            source: 'related',
          );
        })
  ...
  ToolSuggestion? _findAlternative(String currentRoute) {
    const alternatives = <String, List<String>>{
      '/journal': [
        ...
        'Farklı bir bakış açısı için şükran günlüğünü dene.',
      ],
      '/gratitude': [
        '/journal',
        'journal',
        'Switch to your main journal for deeper reflection.',
        'Daha derin yansıma için ana günlüğüne geç.',
      ],
      '/breathing': [
        '/meditation',
        'meditation',
        'Try a guided meditation for a change of pace.',
        'Tempo değişikliği için rehberli bir meditasyon dene.',
      ],
      '/meditation': [
        '/breathing',
        'breathing',
        'A focused breathing exercise might feel better right now.',
        'Odaklı bir nefes egzersizi şimdi daha iyi hissettirebilir.',
      ],
    };
