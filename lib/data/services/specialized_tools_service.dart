...
class SpecializedToolsService {
  ...
  String _getPlanetDirectionMeaning(String planet, CardinalDirection dir) {
    final meanings = {
      'Güneş': 'Kimlik ve yaşam gücünüz ${dir.nameTr} yönünde aktif. Bu yön sizin için enerji ve canlılık kaynağı.',
      'Ay': 'Duygusal ihtiyaçlarınız ${dir.nameTr}\'dan besleniyor. Bu yön huzur ve güvenlik hissi verir.',
      'Merkür': '${dir.nameTr} yönü iletişim ve düşünce aktivitenizi destekliyor. Öğrenme bu yönde kolay.',
      'Venüs': 'Aşk ve güzellik enerjiniz ${dir.nameTr}\'dan geliyor. İlişkiler bu yönde gelişir.',
      'Mars': 'Eylem ve motivasyon gücünüz ${dir.nameTr}\'da yoğunlaşıyor. Bu yön size enerji verir.',
      'Jüpiter': '${dir.nameTr} yönü şans ve büyüme potansiyeli taşıyor. Fırsatlar bu yönden gelir.',
      'Satürn': '${dir.nameTr} yönü yapı ve disiplin gerektiriyor. Burada sabırlı olmanız gerekir.',
    };
    return meanings[planet] ?? '${dir.nameTr} yönü sizin için özel anlam taşıyor.';
  }

  String _getPlanetHomeAdvice(String planet, CardinalDirection dir) {
    final advice = {
      'Güneş': 'Evinizin ${dir.nameTr} kısmını aydınlık tutun. Burada bitkiler ve doğal ışık enerjinizi artırır.',
      'Ay': '${dir.nameTr} köşesini dinlenme alanı olarak kullanın. Burada meditasyon veya yoga yapabilirsiniz.',
      'Merkür': 'Çalışma masanızı veya kitaplığınızı ${dir.nameTr}\'ya yerleştirin. Konsantrasyon artar.',
      'Venüs': '${dir.nameTr} köşesini güzelleştirin. Çiçekler, sanat eserleri veya romantik öğeler ekleyin.',
      'Mars': '${dir.nameTr}\'da egzersiz alanı veya aktif yaşam köşesi oluşturun.',
      'Jüpiter': '${dir.nameTr}\'yı genişletme alanı olarak düşünün. Burada plan ve vizyon çalışması yapın.',
      'Satürn': '${dir.nameTr} köşesini düzenli ve organize tutun. Kaos bu yönde sorun yaratabilir.',
    };
    return advice[planet] ?? '${dir.nameTr} yönünü bilinçli olarak değerlendirin.';
  }

  String _getPlanetTravelAdvice(String planet, CardinalDirection dir) {
    final advice = {
      'Güneş': '${dir.nameTr}\'ya yapacağınız seyahatler kendinizi bulmanıza yardımcı olur.',
      'Ay': '${dir.nameTr} yönündeki yerler duygusal şifa için idealdir. Aile ziyaretleri düşünün.',
      'Merkür': '${dir.nameTr}\'ya eğitim veya iş amaçlı seyahatler verimli olur.',
      'Venüs': '${dir.nameTr} yönü romantik tatiller için mükemmel. Çiftler için önerilir.',
      'Mars': '${dir.nameTr}\'ya macera dolu seyahatler planlayın. Spor aktiviteleri düşünün.',
      'Jüpiter': '${dir.nameTr} yönünde uzak seyahatler şans getirir. Yabancı kültürleri keşfedin.',
      'Satürn': '${dir.nameTr}\'ya planlı ve organize seyahatler yapın. Acele etmeyin.',
    };
  ...
  String _generateHomeAnalysis(List<LocalSpaceLine> lines) {
    ...
    buffer.writeln();
    buffer.writeln('Ev düzeninizi gezegen enerjilerine göre optimize etmek için:');
    buffer.writeln();
    ...
    buffer.writeln();
    buffer.writeln('Bu düzenlemeler, evinizin enerjisini doğum haritanızla uyumlu hale getirir.');
  ...
  String _generateOfficeAnalysis(List<LocalSpaceLine> lines) {
    ...
    buffer.writeln('• Masanızı ${sunLine.direction.nameTr} yönüne bakacak şekilde konumlandırın.');
    buffer.writeln('• İletişim cihazlarınızı ${mercuryLine.direction.nameTr}\'ya yerleştirin.');
    buffer.writeln('• Büyüme hedeflerinizi ${jupiterLine.direction.nameTr} duvarına asın.');
  ...
  String _getChildPersonality(ZodiacSign sign) {
    final profiles = {
      ZodiacSign.aries: 'Koç çocuğunuz enerjik, cesur ve bağımsız bir ruha sahip. '
      ...
      ZodiacSign.taurus: 'Boğa çocuğunuz sakin, kararlı ve güvenilir bir yapıya sahip. '
      ...
      ZodiacSign.gemini: 'İkizler çocuğunuz meraklı, sosyal ve çok yönlüdür. '
      ...
      ZodiacSign.cancer: 'Yengeç çocuğunuz hassas, şefkatli ve sezgisel bir doğaya sahip. '
          'Aile bağları çok önemlidir. Güvenli bir ortam ihtiyacı yüksektir. '
          'Hayal gücü zengindir ve duygusal zekası gelişmiştir.',
      ZodiacSign.leo: 'Aslan çocuğunuz yaratıcı, cömert ve dikkat çekici bir kişiliğe sahip. '
      ...
      ZodiacSign.virgo: 'Başak çocuğunuz analitik, düzenli ve yardımsever bir yapıya sahip. '
      ...
      ZodiacSign.libra: 'Terazi çocuğunuz uyumlu, adil ve sosyal bir kişiliğe sahip. '
      ...
      ZodiacSign.scorpio: 'Akrep çocuğunuz yoğun, kararlı ve sezgisel bir doğaya sahip. '
  ...
  String _getChildLearningStyle(ZodiacSign sign) {
    final styles = {
      ZodiacSign.aries: 'Hareket ederek ve yarışarak öğrenir. Kısa, hızlı aktiviteler tercih eder.',
      ZodiacSign.taurus: 'Pratik uygulamalarla öğrenir. Dokunsal materyaller kullanın. Sabrınız olsun.',
      ZodiacSign.gemini: 'Okuyarak, konuşarak ve çeşitlilikle öğrenir. Farklı yöntemler deneyin.',
      ZodiacSign.cancer: 'Duygusal bağlantı kurarak öğrenir. Hikayeler ve anılar etkilidir.',
      ZodiacSign.leo: 'Performans göstererek ve takdir alarak öğrenir. Yaratıcı projeler verin.',
      ZodiacSign.virgo: 'Sistematik ve detaylı açıklamalarla öğrenir. Adım adım ilerleyin.',
      ZodiacSign.libra: 'İşbirliği yaparak ve tartışarak öğrenir. Partner çalışmaları etkilidir.',
      ZodiacSign.scorpio: 'Derinlemesine araştırarak öğrenir. Bağımsız projeler verin.',
      ZodiacSign.sagittarius: 'Keşfederek ve deneyimleyerek öğrenir. Açık hava aktiviteleri etkilidir.',
      ZodiacSign.capricorn: 'Yapılandırılmış ortamda ve hedeflerle öğrenir. İlerleme gösterin.',
      ZodiacSign.aquarius: 'Teknoloji kullanarak ve grup projelerinde öğrenir. Yenilikçi yöntemler deneyin.',
      ZodiacSign.pisces: 'Görsel ve müzikal materyallerle öğrenir. Hayal gücünü kullanmasına izin verin.',
    };
  ...
  String _getChildSocialStyle(ZodiacSign sign) {
    final styles = {
      ZodiacSign.aries: 'Lider rolünü üstlenir. Rekabetçi oyunları sever. Sabır öğretilmeli.',
      ZodiacSign.taurus: 'Az ama yakın arkadaşlıklar kurar. Paylaşmayı öğrenmeye ihtiyacı var.',
      ZodiacSign.gemini: 'Sosyal kelebek. Çok arkadaş edinir ama yüzeysel kalabilir.',
      ZodiacSign.cancer: 'Seçici arkadaşlıklar. Güvene ihtiyacı var. Evde oynamayı sever.',
      ZodiacSign.leo: 'Popüler ve karizmatik. Dikkat çekmek ister. Drama olabilir.',
      ZodiacSign.virgo: 'Seçici ve yardımsever. Az arkadaşla derin bağlar kurar.',
      ZodiacSign.libra: 'Uyumlu ve diplomatik. Çatışmalardan kaçınır. Adilce oynamayı öğretir.',
      ZodiacSign.scorpio: 'Yoğun ve sadık arkadaşlıklar. Güven kazanmak zaman alır.',
      ZodiacSign.sagittarius: 'Farklı gruplarla arkadaş olur. Maceracı ve eğlenceli.',
      ZodiacSign.capricorn: 'Olgun arkadaşlıklar. Sorumlu rol üstlenir. Rahatlamasına yardımcı olun.',
      ZodiacSign.aquarius: 'Alışılmadık arkadaşlıklar. Grup aktivitelerini sever. Bireyselliğe saygı.',
      ZodiacSign.pisces: 'Empatik ve şefkatli. Sınırları öğretilmeli. Yalnız zamanına ihtiyacı var.',
    };
  ...
  String _getChildEmotionalNeeds(ZodiacSign sun, ZodiacSign? moon) {
    ...
    final needs = {
      ZodiacSign.aries: 'Özgürlük ve bağımsızlık. Engellerle başa çıkmayı öğretmek.',
      ZodiacSign.taurus: 'Güvenlik ve istikrar. Fiziksel konfor ve rutin.',
      ZodiacSign.gemini: 'Zihinsel uyarı ve iletişim. Dinlenildiğini hissetmek.',
      ZodiacSign.cancer: 'Duygusal güvenlik ve aile bağı. Kabul edilmek.',
      ZodiacSign.leo: 'Tanınma ve takdir. Özel hissetmek.',
      ZodiacSign.virgo: 'Düzen ve yararlı olma hissi. Mükemmel olmak zorunda değil.',
  ...
  List<String> _getChildChallenges(ZodiacSign sign) {
    final challenges = {
      ...
      ZodiacSign.cancer: ['Aşırı hassasiyet', 'Bağımlılık', 'Ruh hali değişimleri'],
      ZodiacSign.leo: ['Dikkat ihtiyacı', 'Kibir', 'Eleştiri alma'],
      ZodiacSign.virgo: ['Mükemmeliyetçilik', 'Endişe', 'Öz eleştiri'],
      ZodiacSign.libra: ['Karar verme', 'Çatışmadan kaçınma', 'İnsanları memnun etme'],
      ...
      ZodiacSign.capricorn: ['Aşırı ciddiyet', 'İş-oyun dengesi', 'Duygusal ifade'],
  ...
  String _getParentingTips(ZodiacSign sign) {
    final tips = {
      ZodiacSign.aries: 'Fiziksel aktiviteye bolca alan tanıyın. Rekabetçi oyunlar oynayın ama '
          'kaybetmeyi de öğretin. Bağımsız kararlar almasına izin verin.',
      ZodiacSign.taurus: 'Rutin oluşturun ve değişiklikleri yavaş yavaş tanıtın. '
          'Dokunsal deneyimler sunun. Sabrınızı koruyun, acele etmeyin.',
      ZodiacSign.gemini: 'Çeşitli aktiviteler sunun ve merakını besleyin. '
          'Konuşmalarınızı ilgi çekici tutun. Odaklanma becerileri geliştirin.',
      ZodiacSign.cancer: 'Güvenli ve sıcak bir ev ortamı yaratın. '
          'Duygularını ifade etmesine izin verin. Aile ritüelleri oluşturun.',
      ZodiacSign.leo: 'Yaratıcılığını destekleyin ve başarılarını takdir edin. '
          'Sahne performansı fırsatları sunun. Alçakgönüllülük öğretin.',
      ZodiacSign.virgo: 'Yapıcı geri bildirim verin, eleştirmekten kaçının. '
          'Düzen ihtiyacını anlayın. Mükemmel olmak zorunda olmadığını öğretin.',
      ZodiacSign.libra: 'Karar verme pratiği yaptırın. Adil davranın ve tutarlı olun. '
          'Sosyal becerilerini destekleyin ama kendi sesini bulmasına yardımcı olun.',
      ZodiacSign.scorpio: 'Güvenini kazanın ve gizliliğine saygı gösterin. '
          'Dürüst olun çünkü yalanı anlar. Duygusal güvenlik sağlayın.',
      ZodiacSign.sagittarius: 'Özgürlük alanı tanıyın ve macera fırsatları sunun. '
          'Sorularını ciddiye alın. Sınırları nazikçe ama tutarlı bir şekilde koyun.',
      ZodiacSign.capricorn: 'Sorumluluk verin ama aşırı yüklemeyin. Eğlenceyi teşvik edin. '
          'Başarılarını kutlayın ve çabasını takdir edin.',
      ZodiacSign.aquarius: 'Farklılığını kutlayın ve bireyselliğine saygı gösterin. '
          'Mantıkla yaklaşın. Sosyal bilinç geliştirmesine yardımcı olun.',
      ZodiacSign.pisces: 'Hayal gücünü destekleyin ama sınırlar da koyun. '
  ...
  String _getFamilyDynamics(Element dominant, List<FamilyMember> members) {
    final dynamics = {
      Element.fire: 'Aileniz enerji, tutku ve heyecan dolu! Aktivite ve macera ailenizin '
          'bağ kurma yollarından biri. Rekabet olabilir ama coşkunuz bulaşıcı.',
      Element.earth: 'Aileniz istikrar, güvenlik ve pratiklik üzerine kurulu. Gelenekler '
          'önemli ve maddi konfor aile mutluluğunun bir parçası.',
      Element.air: 'Aileniz iletişim, fikir alışverişi ve sosyal aktiviteler üzerine odaklı. '
          'Konuşmalar ve tartışmalar aile bağınızı güçlendiriyor.',
      Element.water: 'Aileniz derin duygusal bağlar ve sezgisel anlayışa sahip. '
  ...
  List<String> _getFamilyStrengths(Element dominant, List<FamilyMember> members) {
    final strengths = {
      Element.fire: ['Enerji ve coşku', 'Cesaret', 'İlham vericilik', 'Motivasyon'],
  ...
  String _getFamilyAdvice(Element dominant, Random random) {
    final advice = {
      Element.fire: 'Bugün bir aile spor aktivitesi planlayın. Enerjinizi birlikte harcayın!',
      Element.earth: 'Birlikte yemek yapmak veya bahçe işleri aile bağınızı güçlendirir.',
      Element.air: 'Bir aile oyunu oynayın veya birlikte bir konu hakkında sohbet edin.',
      Element.water: 'Duygusal paylaşım zamanı. Herkesin duygularını ifade etmesine izin verin.',
    };
